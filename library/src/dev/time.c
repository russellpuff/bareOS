...
void dt_to_string(datetime dt, char* out, uint8_t len) {
	...
	sprintf((byte*)ptr, "%u", dt.minute);
	if (dt.minute < 10) min[0] = '0';

	/* Prep second */
	char sec[3];
	ptr = dt.second < 10 ? sec + 1 : sec;
	sprintf((byte*)ptr, "%u", dt.second);
	if (dt.second < 10) sec[0] = '0';

	/* Null terminate */
	sec[2] = min[2] = mer[2] = hr[2] = '\0';

	sprintf((byte*)out, "%s %u %u %s:%s:%s %s %s", month, dt.day, dt.year, hr, min, sec, mer, tz);
}
