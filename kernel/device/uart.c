 ...
 */
void init_uart(void) {
  uint32_t* plic_prio_addr = (uint32_t*)PA_TO_KVA(UART_PRIO_ADDR);
  uint32_t* plic_enabled_addr = (uint32_t*)PA_TO_KVA(EXTERNAL_ENABLED_ADDR);
  ...
  *plic_enabled_addr = *plic_enabled_addr | UART_ENABLE;    /*    Enable the UART on the PLIC    */
  ...
  uart = (byte*)PA_TO_KVA(UART0_CFG_REG);            /*  Set the location of the UART in memory     */
