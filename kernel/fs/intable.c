...
#define IN_PER_BLOCK (uint16_t)(fsd->device.block_size / sizeof(inode_t))

static inline byte* get_block(uint16_t index) {
	return fsd->device.ramdisk + (index * fsd->device.block_size);
}
...
uint16_t in_find_free(void) {
	for (byte blk_idx = 0; blk_idx < fsd->super.intable_numblks; ++blk_idx) {
		inode_t* block = get_block(fsd->super.intable_blocks[blk_idx]);
	...
	if (fsd->super.intable_numblks < MAX_INTABLE_BLOCKS) {
		...
		fsd->super.intable_blocks[fsd->super.intable_numblks++] = blk_idx;
		inode_t* block = get_block(blk_idx);
		memset((byte*)block, 0, fsd->device.block_size); /* Zero whole block in case of leftover junk. */
		block->type = BUSY; /* Prevent theft. */
		write_super();
		return ((uint16_t)fsd->super.intable_numblks - 1) * IN_PER_BLOCK;
	}
